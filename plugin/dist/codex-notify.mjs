...
async function runNotify(raw, deferMs = notifyDeferMs(), sleep = (ms) => new Promise((r) => setTimeout(r, ms))) {
  try {
    ...
    const envelope = await buildEnvelope(input, machine, now, title, config.e2eKey, false, "codex", startedAt, turnStartedAt, undefined, model, plan, attentionKind, pendingPlanPicker ? evidence.plan : undefined);
    if (!envelope)
      return;
